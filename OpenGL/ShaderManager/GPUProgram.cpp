...
bool GPUProgram::load(const char* vertexProgram, const char* fragmentProgram, const char* geometryProgram, int GS_inputPrimitiveType, int GS_outputPrimitiveType, int GS_maxVerticesOut)
{
	...
	/*    //specify required parameter by geometry shader
	    ...
	    if(GS_inputPrimitiveType==GL_POINTS)
		std::cout<<"ok GS_inputPrimitiveType"<<std::endl;
	    if(GS_outputPrimitiveType==GL_TRIANGLE_STRIP)
		std::cout<<"ok GS_outputPrimitiveType"<<std::endl;
	    std::cout<<"GS_maxVerticesOut="<<GS_maxVerticesOut<<std::endl;*/
...
GLuint GPUProgram::getUniformLocation(const char* uniformName) const
{
	GLuint ret;
    glUseProgramObjectARB(this->id);
...
GLuint GPUProgram::getAttributeLocation(char* attributeName) const
{
	GLuint ret;
    glUseProgramObjectARB(this->id);
	ret = glGetAttribLocationARB( (GLhandleARB)this->id, (const GLchar*)attributeName);
    glUseProgramObjectARB(0);
	return ret;
}
